...
pub fn probe(path: &Path) -> Result<Schema3Probe, String> {
 if !path.is_file() { return Err("input-not-regular-file".into()) }
 let before=std::fs::read(path).map_err(|_|"input-unreadable")?;
 let digest=Sha256::digest(&before);
 let temporary=std::env::temp_dir().join(format!("message-schema3-probe-{}-{}",std::process::id(),std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map_err(|_|"private-copy-unavailable")?.as_nanos()));
 std::fs::create_dir(&temporary).map_err(|_|"private-copy-unavailable")?;
 let copy=temporary.join("messenger.sema");
 std::fs::write(&copy,&before).map_err(|_|"private-copy-unavailable")?;
 let result=probe_copy(&copy);
 let _=std::fs::remove_dir_all(&temporary);
 let after=std::fs::read(path).map_err(|_|"source-changed")?;
 if Sha256::digest(&after)!=digest { return Err("source-changed".into()) }
 result
}
