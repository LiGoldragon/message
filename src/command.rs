...
impl CommandLine {
    ...
    pub fn run(&self, mut output: impl Write) -> Result<()> {
        ...
        if let [
            ...
            && command == "cluster"
            && body_flag == "--body-file"
            && routes_flag == "--route-config"
            && to_flag == "--to"
        {
            let body = fs::read_to_string(body_path)?;
            let canonical = crate::cluster::verify(header, &body)
                .map_err(|detail| Error::InvalidCommandArgument { detail })?;
            let (flow_identifier, session_identifier) = target
                .split_once('@')
                .filter(|(flow, session)| !flow.is_empty() && !session.is_empty())
                .ok_or_else(|| Error::InvalidCommandArgument {
                    detail: "--to must be FLOW@SESSION".into(),
                })?;
            ...
            let receipt =
                route.deliver(&header_file.path, Path::new(body_path), session_identifier)?;
            writeln!(
                output,
                "{}",
                serde_json::to_string(&receipt).map_err(|error| Error::InvalidCommandArgument {
...
impl FlowRoutes {
    fn read(path: &Path) -> Result<Self> {
        serde_json::from_str(&fs::read_to_string(path)?).map_err(|error| {
            Error::InvalidCommandArgument {
                detail: format!("parse Flow route config {}: {error}", path.display()),
            }
        })
    }
    fn exact(&self, flow: &str, session: &str) -> Result<&FlowRoute> {
        let matches = self
            .routes
            .iter()
            .filter(|route| route.flow_identifier == flow && route.session_identifier == session)
            .collect::<Vec<_>>();
        let [route] = matches.as_slice() else {
            return Err(Error::InvalidCommandArgument {
                detail: "Flow route config must contain exactly one selected target".into(),
            });
        };
        ...
        if !ready {
            return Err(Error::InvalidCommandArgument {
                detail: "selected Flow route is not a configured ready Claude or known-state Codex prompt-relay bridge".into(),
            });
        }
        Ok(route)
    }
...
impl FlowRoute {
    fn deliver(&self, header: &Path, body: &Path, session: &str) -> Result<Value> {
        ...
        command
            ...
            .arg("--source")
            .arg(body)
            .args(["--source-format", "peer-file", "--datom-file"])
            .arg(header);
        match self.harness.as_str() {
            "claude" => {
                command.args(["--session-short", session]);
            }
            "codex" => {
                let socket =
                    self.socket_path
                        .as_ref()
                        .ok_or_else(|| Error::InvalidCommandArgument {
                            detail: "configured Codex route has no Flow-supplied socket_path"
                                .into(),
                        })?;
                command
                    .args(["--thread-id", session, "--socket"])
                    .arg(socket);
            }
            _ => {
                return Err(Error::InvalidCommandArgument {
                    detail: "unsupported Flow bridge harness".into(),
                });
            }
        }
        let output = command
            .output()
            .map_err(|error| Error::InvalidCommandArgument {
                detail: format!(
                    "start configured prompt-relay {}: {error}",
        ...
        if !output.status.success() {
            ...
            return Err(Error::InvalidCommandArgument {
                detail: format!("configured prompt-relay refused: {error}"),
            });
        }
        let receipt: Value = serde_json::from_slice(&output.stdout).map_err(|error| {
            Error::InvalidCommandArgument {
                detail: format!("parse prompt-relay receipt: {error}"),
            }
        })?;
        ...
        if !accepted {
            return Err(Error::InvalidCommandArgument {
                detail: "configured prompt-relay did not acknowledge the selected route write"
                    .into(),
            });
        }
        Ok(receipt)
...
impl TemporaryHeader {
    fn write(header: &str) -> Result<Self> {
        let path =
            std::env::temp_dir().join(format!("message-cluster-header-{}", std::process::id()));
