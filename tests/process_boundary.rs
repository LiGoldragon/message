...
/// The daemon receives a fixture-only HOME.  The production provisional flow
/// registry discovers marker roots relative to HOME, so a test must not
/// create markers under the calling user's actual flow lanes.
