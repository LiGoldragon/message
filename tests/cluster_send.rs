...
fn cluster_send_uses_one_flow_configured_prompt_relay_and_reports_its_acknowledgment() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_message"))
        .args([
            "cluster",
            header,
            "--body-file",
    ...
    assert_eq!(
        serde_json::from_slice::<serde_json::Value>(&output.stdout).unwrap()["kind"],
        "claude-bytes-written-to-pty"
    );
    assert_eq!(fs::read_to_string(capture).unwrap(), header);
}

#[test]
fn cluster_send_queues_the_next_turn_for_a_busy_claude_flow() {
    let directory = tempfile::tempdir().unwrap();
    let body = directory.path().join("body.txt");
    fs::write(&body, BODY).unwrap();
    let header = format!(
        "Peer.{{ {{ sender source-session }} source-event {} {} «{}» }}",
        body.display(),
        "aba2570815169d04bc0b454c3e107cbce15af228ca43707937637f25e738ab2b",
        BODY
    );
    let adapter = directory.path().join("prompt-relay");
    fs::write(
    ...
    fs::set_permissions(&adapter, fs::Permissions::from_mode(0o700)).unwrap();
    let routes = directory.path().join("routes.json");
    fs::write(&routes, serde_json::json!({"routes":[{"flow_identifier":"target","session_identifier":"target-session","harness":"claude","readiness":"busy","endpoint":adapter}]}).to_string()).unwrap();
    let output = Command::new(env!("CARGO_BIN_EXE_message"))
        .args([
            "cluster", &header, "--body-file", body.to_str().unwrap(),
            "--route-config", routes.to_str().unwrap(), "--to", "target@target-session",
        ])
        .output()
        .unwrap();
...
#[test]
fn cluster_send_accepts_a_busy_codex_route_with_a_turn_acknowledgment() {
    let directory = tempfile::tempdir().unwrap();
    let transcript = directory.path().join("source.jsonl");
    fs::write(&transcript, serde_json::json!({ "type":"user", "uuid":"source", "sessionId":"cf7879-session", "timestamp":"2026-09-16T00:00:00Z", "message":{"content":BODY} }).to_string()).unwrap();
    let rendered = String::from_utf8(relay(&transcript).output().unwrap().stdout).unwrap();
    ...
    let routes = directory.path().join("routes.json");
    fs::write(&routes, serde_json::json!({"routes":[{"flow_identifier":"codex","session_identifier":"thread-full-id","harness":"codex","readiness":"busy","endpoint":adapter,"socket_path":socket}]}).to_string()).unwrap();
    let output = Command::new(env!("CARGO_BIN_EXE_message"))
        .args([
            "cluster",
            header,
    ...
    assert_eq!(
        serde_json::from_slice::<serde_json::Value>(&output.stdout).unwrap()["turn_id"],
        "turn-1"
    );
}
